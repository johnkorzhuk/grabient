...
export const STYLE_LABELS: Record<(typeof PALETTE_STYLES)[number], string> =
  {
...
/**
 * Maps styles that require non-CSS rendering to their CSS fallback styles.
 * Used for loading and error states when the actual rendering approach is unavailable.
 */
export const FALLBACK_STYLES: Partial<
  Record<(typeof PALETTE_STYLES)[number], (typeof PALETTE_STYLES)[number]>
> = {
  deepFlow: 'linearSwatches',
} as const;

export const DEFAULT_STYLE: (typeof PALETTE_STYLES)[number] =
  'linearGradient';
...
export const sizeWithAutoValidator = v.union([v.literal('auto'), sizeValidator]);
