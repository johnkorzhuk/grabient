...
export function relativeAge(ms: number, nowMs: number): string {
  ...
  if (s < 3600) return duration(Math.max(1, Math.floor(s / 60)), "minute");
  if (s < 86_400) return duration(Math.floor(s / 3600), "hour");
  if (s < 86_400 * 30) return duration(Math.floor(s / 86_400), "day");
  if (s < 86_400 * 365) {
    return duration(Math.floor(s / (86_400 * 30)), "month");
  }
