...
function pageHeader(current: string, stampText: string, email: string, state: DashboardState): string {
  return `<header>
...
export function reportsArchivePage(
  items: ReportMeta[],
  options: { kind?: string; nextCursor: string | null; stamp: string; email: string; state: DashboardState },
): string {
  const filters = [
    { key: "", label: "All" },
    ...
    .map((f) => {
      ...
      return `<a href="${esc(href("/reports", options.state, { kind: f.key || undefined }))}"${active ? ' aria-current="true"' : ""} class="rounded-md px-2 py-0.5 text-xs font-bold transition-colors ${active ? "bg-ink text-surface" : "text-ink-muted hover:text-ink"}">${f.label}</a>`;
    })
    .join("");
  ...
  ${pageHeader("/reports", options.stamp, options.email, options.state)}
  ...
  ${
    options.nextCursor
      ? `<p class="mt-6"><a href="${esc(href("/reports", options.state, { before: options.nextCursor }))}" class="text-xs text-ink-muted underline hover:text-ink">Older →</a></p>`
      : ""
  }
</main>`,
  );
