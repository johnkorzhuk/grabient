...
export async function downloadPNGGridAsync(options: PNGGridOptions): Promise<void> {
  try {
    const blob = await generatePNGGridBlobAsync(options);
...
export async function copyPNGGridToClipboardAsync(
  options: PNGGridOptions,
): Promise<void> {
  try {
    const blob = await generatePNGGridBlobAsync(options);
