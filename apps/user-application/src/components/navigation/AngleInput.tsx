...
interface AngleInputProps {
    ...
    onPreviewChange?: (angle: number | null) => void;
}
...
export function AngleInput({
    ...
    const handleLabelClick = (e: React.MouseEvent) => {
        ...
        setIsDirty(false);
        setLocalValue(
            value === "auto" ? DEFAULT_ANGLE.toString() : value.toString(),
        );
        setTimeout(() => {
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (newValue === "" || newValue === "auto" || newValue === "angle") {
            ...
            if (!isNaN(numValue) && validateAngle(numValue)) {
                ...
                onPreviewChange?.(formattedAngle);
            }
        }
    };
    ...
    const handleInputKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === "ArrowUp" || e.key === "ArrowDown") {
            ...
            if (newValue > MAX_ANGLE) {
                newValue = MIN_ANGLE + ((newValue - MIN_ANGLE) % range);
            } else if (newValue < MIN_ANGLE) {
                newValue = MAX_ANGLE - ((MIN_ANGLE - newValue) % range);
            }

            if (validateAngle(newValue)) {
                const formattedAngle = parseFloat(newValue.toFixed(1));
                setLocalValue(formattedAngle.toString());
                onPreviewChange?.(formattedAngle);
                setIsDirty(true);
            }
        }
    ...
    const handleValueClick = async (clickedAngle: number) => {
        const newAngle = clickedAngle === value ? "auto" : clickedAngle;
        ...
        // Navigate with updated angle search param
        await router.navigate({
            to: location.pathname,
            search: (prev) => ({
                ...prev,
            ...
            <PopoverTrigger asChild>
                <button
                    ...
                    className={cn(
                        ...
                        "w-[70px] md:w-[90px] justify-between",
