...
#
# Requires: claude CLI on PATH, and env vars
...
while true; do
  ...
  echo "[$(date +%H:%M:%S)] iteration $iteration mode=$mode run=$run_id (pending pairs: $pending)"
  ...
  if timeout "$ITERATION_TIMEOUT" claude -p "/$mode run_id=$run_id" \
    --allowedTools "Bash(curl:*),Bash(npx tsx harness/*),Read" \
    --max-turns 50 >>"$log_file" 2>&1; then
    status="done"
    consecutive_failures=0
  else
    status="failed"
